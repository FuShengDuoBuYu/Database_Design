...
- java
- mysql(使用远程服务器,非本地,可能会慢一点)

#### 运行程序说明

- 在jar包之外的configInfo.txt中配置数据库连接信息
  - 第一行为数据库连接地址
  - 第二行为数据库用户名
  - 第三行为数据库密码
- 先解压文件压缩包,然后进入代码目录,运行jar包即可
```bash
unzip DBLAB1

cd DataBase_LAB1

java -jar DataBase_LAB1.jar
...
##### 问题三:
###### student表的数据有部分问题
- 在student中,有些学生的号码位数和其他学生不同,在这里,我将这些学生的号码前面补0,使其位数和其他学生一致
- 在student中,有些学生的姓名中含有空格,我在这里将这些学生的姓名中的空格去掉

更好的做法是将这些信息写入日志文件中,方便后期查看,但是考虑到lab1,并没有复杂的功能等,因此只是在控制台打印出来
...
- 不允许不符合完整性要求的数据插入数据库
  - 但是对于多条数据一同插入时,不应当某一条不符合要求的数据导致所有数据的插入都失败
- 为不符合完整性要求的数据添加默认值
...
- 对于不符合规则的原始数据,如果其他字段的内容需要保留,那么就给予不符合规则的字段一个原始值(默认值)
- 如果其他字段不需要保留,那么就给编程者提示插入失败后舍弃其他字段的信息即可
- 对于多条同级的同类的数据的插入,不应让某条数据的处理结果影响其他数据的处理
